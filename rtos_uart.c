...
static rtos_uart_handle_t uart_handles[NUMBER_OF_SERIAL_PORTS] = { 0 };
...
static void fsl_uart_callback(UART_Type *base, uart_handle_t *handle, status_t status, void *userData)
{
	...
	if (kStatus_UART_TxIdle == status) {
		if (UART0 == base) {
			xSemaphoreGiveFromISR(uart_handles[rtos_uart0].tx_sem,
					&xHigherPriorityTaskWoken);
		} else {
			xSemaphoreGiveFromISR(uart_handles[rtos_uart1].tx_sem,
					&xHigherPriorityTaskWoken);
		}
	} // end if
	...
	if (kStatus_UART_RxIdle == status) {
		if (UART0 == base) {
			xSemaphoreGiveFromISR(uart_handles[rtos_uart0].rx_sem,
					&xHigherPriorityTaskWoken);
		} else {
			xSemaphoreGiveFromISR(uart_handles[rtos_uart1].rx_sem,
					&xHigherPriorityTaskWoken);
		}
	} // end if
...
rtos_uart_flag_t rtos_uart_init(rtos_uart_config_t config)
{
	...
	if (config.uart_number < NUMBER_OF_SERIAL_PORTS) {
		if (!uart_handles[config.uart_number].is_init) {
			...
			PORT_SetPinMux(get_port_base(config.port), config.rx_pin,
					config.pin_mux);
			PORT_SetPinMux(get_port_base(config.port), config.tx_pin,
					config.pin_mux);
			...
			if (rtos_uart0 == config.uart_number) {
				UART_Init(get_uart_base(rtos_uart0), &fsl_config,
						CLOCK_GetFreq(UART0_CLK_SRC));
				NVIC_SetPriority(UART0_RX_TX_IRQn, 5);
			} else {
				UART_Init(get_uart_base(rtos_uart1), &fsl_config,
						CLOCK_GetFreq(UART1_CLK_SRC));
				NVIC_SetPriority(UART1_RX_TX_IRQn, 5);
			}

			UART_TransferCreateHandle(get_uart_base(config.uart_number),
					&uart_handles[config.uart_number].fsl_uart_handle,
